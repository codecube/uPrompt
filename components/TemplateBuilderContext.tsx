...
import { Block, PromptHistory, SavedBlock } from '../types';
...
export const TemplateBuilderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadPromptFromHistory = useCallback((prompt: PromptHistory) => {
    const loadedBlocks = prompt.blocks.map((savedBlock: SavedBlock, index: number) => {
...
export const ClientTemplateBuilderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return (
